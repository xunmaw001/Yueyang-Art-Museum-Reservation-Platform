	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('caozuorizhi', this.caozuorizhiId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.caozuorizhiPhoto ? this.detail.caozuorizhiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('caozuorizhiYuyue', {
									yonghuId: _this.user.id,
									caozuorizhiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/caozuorizhiYuyue/list');
							}
						}
					});
				},
