		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.meishuguanNewMoney>1000? 'width: 450rpx;':detail.meishuguanNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.meishuguanNewMoney}}</text>
				</view>
			</view>
			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.meishuguanName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.meishuguanContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						<view style="width: 40%;display: flex;align-items: center;flex-wrap: wrap;justify-content: space-evenly;">
								<!-- 收藏 -->
						...
						<view v-if="new Date(detail.jieshuTime).getTime() > new Date().getTime()"  style="display: flex; width: 60%;color: #FFFFFF;align-items: center;">
							<uni-number-box :min="1" v-model="buyNumber" />
							<view @tap="onSubmit" :style="[{backgroundImage:'linear-gradient(120deg,'+ btnColor[0] +','+ btnColor[1] +')'}]" style="width: 95%;height: 80rpx;line-height: 80rpx;text-align: center;border-radius: 40rpx;">
								立即预约
							</view>
						</view>
                        <!--两个按钮
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishuguanId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getmeishuguanCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        meishuguanId: this.meishuguanId,
                        yonghuId: this.user.yonghuId,
						meishuguanLiuyanPingfenNumber: this.scoreData,
                        meishuguanLiuyanText: this.commentData,
                    }
                    await this.$api.save(`meishuguanLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishuguan', this.meishuguanId);
                    this.detail = res.data
					this.detail.meishuguanContent = this.detail.meishuguanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.meishuguanPhoto ? this.detail.meishuguanPhoto.split(",") : [];
				},
				...
				async getmeishuguanCollection() {
					...
					let res = await this.$api.page(`meishuguanCollection`, params);
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`meishuguanCollection`, params);
					if (res.data.list.length == 1) {
						let meishuguanCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('meishuguanCollection', JSON.stringify([meishuguanCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getmeishuguanCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishuguanCollection', {
									yonghuId: _this.user.id,
									meishuguanId: _this.detail.id,
									meishuguanCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('meishuguanLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						meishuguanId: this.meishuguanId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('meishuguan', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishuguanYuyue', {
									yonghuId: _this.user.id,
									meishuguanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishuguanYuyue/list');
							}
						}
					});
				},
