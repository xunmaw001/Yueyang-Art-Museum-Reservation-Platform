				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.meishuguanCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishuguanCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishuguanCollection', this.meishuguanCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.meishuguanCollectionPhoto ? this.detail.meishuguanCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishuguanCollectionYuyue', {
									yonghuId: _this.user.id,
									meishuguanCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishuguanCollectionYuyue/list');
							}
						}
					});
				},
