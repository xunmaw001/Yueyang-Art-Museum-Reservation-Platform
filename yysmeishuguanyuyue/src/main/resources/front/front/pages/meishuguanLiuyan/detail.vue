	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishuguanLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishuguanLiuyan', this.meishuguanLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.meishuguanLiuyanPhoto ? this.detail.meishuguanLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishuguanLiuyanYuyue', {
									yonghuId: _this.user.id,
									meishuguanLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishuguanLiuyanYuyue/list');
							}
						}
					});
				},
