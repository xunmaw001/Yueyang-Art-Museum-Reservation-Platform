	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.meishuguanLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('meishuguanLiuyan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('meishuguanLiuyan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <view style="font-size: 30rpx;display: flex;justify-content: space-between;">
                    ...
                    <view style="float: right;padding: 10rpx 10rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></view>
                        ...
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.meishuguanLiuyanText}}</text>
                        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`meishuguanLiuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('meishuguanLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.meishuguanLiuyanShenheTime=shijian;
                            await _this.$api.update(`meishuguanLiuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.meishuguanLiuyanText) {
                    searchForm['meishuguanLiuyanText'] = this.searchForm.meishuguanLiuyanText
                }

                let res = await this.$api.page(`meishuguanLiuyan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
