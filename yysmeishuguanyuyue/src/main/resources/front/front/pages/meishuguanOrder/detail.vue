	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishuguanOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishuguanOrder', this.meishuguanOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.meishuguanOrderPhoto ? this.detail.meishuguanOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishuguanOrderYuyue', {
									yonghuId: _this.user.id,
									meishuguanOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishuguanOrderYuyue/list');
							}
						}
					});
				},
