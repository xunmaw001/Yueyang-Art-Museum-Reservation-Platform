            ...
            <view v-for="(item,index) in list" :key="index">
                <view class="bg-white margin-sm padding-sm solids-bottom radius text-content">
                    ...
                    <view class="flex justify-end" style="font-size: 24upx;">
                        ...
                        <view v-if="item.meishuguanOrderTypes==103">
                            <view
                                @tap="ruguanqiandao(item.id)"
                                class="round cu-btn lines-grey mid  margin-right-sm">入馆签到</view>
                        </view>
						<view v-if="item.meishuguanOrderTypes==104">
						    <view
						        @tap="qiantui(item.id)"
						        class="round cu-btn lines-grey mid  margin-right-sm">出馆签退</view>
						</view>
                    </view>
                </view>
            </view>
				...
				uni.showModal({
				    ...
				    success: async function(res) {
				        if(res.confirm){
							let data = {
								id:id,
								meishuguanOrderTypes:104
							}
							 await _this.$api.update(`meishuguanOrder`, data);
