			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.zhanpinName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.zhanpinVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.zhanpinContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhanpinId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getzhanpinCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        zhanpinId: this.zhanpinId,
                        yonghuId: this.user.yonghuId,
						zhanpinLiuyanPingfenNumber: this.scoreData,
                        zhanpinLiuyanText: this.commentData,
                    }
                    await this.$api.save(`zhanpinLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('zhanpin', this.zhanpinId);
                    this.detail = res.data
					this.detail.zhanpinContent = this.detail.zhanpinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.zhanpinPhoto ? this.detail.zhanpinPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getzhanpinCollection() {
					let params = {
						page: 1,
						limit: 1,
						zhanpinId: this.zhanpinId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`zhanpinCollection`, params);
					if (res.data.list.length == 1) {
						let zhanpinCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('zhanpinCollection', JSON.stringify([zhanpinCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getzhanpinCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhanpinCollection', {
									yonghuId: _this.user.id,
									zhanpinId: _this.detail.id,
									zhanpinCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('zhanpinLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						zhanpinId: this.zhanpinId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('zhanpin', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhanpinYuyue', {
									yonghuId: _this.user.id,
									zhanpinId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhanpinYuyue/list');
							}
						}
					});
				},
