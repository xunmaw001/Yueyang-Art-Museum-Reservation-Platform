	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.zhanpinName" type="text" placeholder="展品名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('zhanpin','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('zhanpin','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='zhanpinTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in zhanpinTypesList" :key="index" :class="zhanpinTypes===item.codeIndex?'tabActive':''"
                                  @tap="zhanpinTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="uni-product-list"
    ...
    <view @tap="onDetailTap(product)" class="uni-product"
        ...
        <view v-if="product.zhanpinNewMoney" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"25rpx","lineHeight":"40rpx","borderStyle":"solid"}'>
            <text style="font-size: 30rpx;">￥</text>{{product.zhanpinNewMoney}}
        </view>
        <view v-else-if="product.zhanpinValue" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"25rpx","lineHeight":"40rpx","borderStyle":"solid"}'>
            {{product.zhanpinValue}}
        </view>
        <image
                ...
                :src="product.zhanpinPhoto?baseUrl+product.zhanpinPhoto.split(',')[0]:''"></image>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('zhanpin','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改
            </text>
            <text v-if="isAuth('zhanpin','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 展品类型 字段 字典表查询方法
            async zhanpinTypesSelect(){
                let zhanpinTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'zhanpin_types',
                });
                zhanpinTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.zhanpinTypesList = zhanpinTypes.data.list
            },
            //当前表的 展品类型字段类别搜索
			zhanpinTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.zhanpinTypes != 0) {
                    params.zhanpinTypes = this.zhanpinTypes
                }
                let _this = this
                let res = await this.$api.list(`zhanpin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhanpin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.zhanpinShenheTime=shijian;
                            await _this.$api.update(`zhanpin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.zhanpinName) {
                    searchForm['zhanpinName'] = this.searchForm.zhanpinName
                }

                let res = await this.$api.list(`zhanpin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.zhanpinContent!=null){
                        item.zhanpinContent = item.zhanpinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
