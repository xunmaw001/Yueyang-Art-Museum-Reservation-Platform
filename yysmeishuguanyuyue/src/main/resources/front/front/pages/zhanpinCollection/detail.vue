	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhanpinCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('zhanpinCollection', this.zhanpinCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhanpinCollectionPhoto ? this.detail.zhanpinCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhanpinCollectionYuyue', {
									yonghuId: _this.user.id,
									zhanpinCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhanpinCollectionYuyue/list');
							}
						}
					});
				},
