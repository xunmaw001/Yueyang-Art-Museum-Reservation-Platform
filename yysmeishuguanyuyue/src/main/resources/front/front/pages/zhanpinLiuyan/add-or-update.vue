        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.zhanpinLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`zhanpinLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.zhanpinId){
                this.ruleForm.zhanpinId = options.zhanpinId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.zhanpinLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`zhanpinLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`zhanpinLiuyan`, this.ruleForm);
                }
