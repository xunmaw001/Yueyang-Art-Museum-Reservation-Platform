	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhanpinLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('zhanpinLiuyan', this.zhanpinLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhanpinLiuyanPhoto ? this.detail.zhanpinLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhanpinLiuyanYuyue', {
									yonghuId: _this.user.id,
									zhanpinLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhanpinLiuyanYuyue/list');
							}
						}
					});
				},
